 ...
 *
 * Retained source path: web/omni-console/src/lib/automation-action-policy.ts
 * System layer: Productization Boundary Layer - typed UI client, fail-closed state, and read-model helpers
 * Review role: operator policy and action-gating boundary.
 *
 * Structural signals retained from the implementation file:
 * - exported interfaces/types/classes: AutomationActionPolicy, AutomationActionPolicyContext, AutomationRunMode, AutomationKillSwitchScope, AutomationActionId, AutomationRiskDirection, AutomationExecutionMode, AutomationOperatingPosture.
 * - exported functions/components/constants: resolveLaunchActionId, resolveKillSwitchActionId, getAutomationActionPolicy, listAutomationActionPolicies, formatAutomationRunModeLabel, formatAutomationRiskDirection, formatAutomationExecutionMode, formatAutomationOperatingPosture, formatAutomationScopeLabel, formatAutomationRuleLabel, AUTOMATION_KILL_SWITCH_SCOPE_OPTIONS.
 * - local helper functions: buildLaunchPolicy, buildKillSwitchPolicy.
 *
