...
| Layer 2 - Provider / Model / Runtime Engines | Feature/external-factor/signal/economic providers, LabelOracle, ModelZoo, Calibration/OOD, DecisionRuntime, ExecutionReplayEngine, PortfolioEngine | `source/omni_station/research_os/engines/research_core_native.py`, `source/gpm/expr/hybrid_executor.py` |
| Layer 1 - Data / Compute / Artifact Infrastructure | Market-data inputs, Parquet/Arrow/manifests, cache, partitioning, atomic writes, station runner, progress events, local/native/GPU compute | `source/omni_station/research_os/data_plane/manifest.py`, `source/gpm/artifact/io.py` |
...
| Providers and engines | Feature, factor, model, calibration, replay, decision runtime, and portfolio engines | `source/gpm/expr/engine_planner.py`, `redacted_capabilities/native_compute_infrastructure/fused_factor_kernel.py` |
| Infrastructure | Physical storage, compute, artifacts, cache, checkpoints, progress, and atomic writes | `source/gpm/artifact/manifest.py`, `redacted_capabilities/data_compute_artifact_infrastructure/lineage_manifest_store.py` |
